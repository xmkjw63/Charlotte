...
<div class="k-main <?php echo Charlotte_option('top_img_switch', true) ? 'banner' : 'color' ?>" style="background:<?php echo Charlotte_option('g_background', '#f5f5f5'); ?>">
    <div class="container mainpjax">
        <div class="row">
            <div class="<?php echo $col_array[Charlotte_option('g_article_widgets', 'two_side')] ?> details">
                <?php if (have_posts()) : the_post();
                    ...
                    <div class="article">
                            ...
                            <div class="breadcrumb-box">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item">
                                        <a class="text-dark" href="<?php echo home_url(); ?>"> <?php _e('首页', 'Charlotte'); ?></a>
                                    </li>
                                    <?php
                                    ...
                                    echo $breadcrumb;
                                    ?>
                                    <li class="breadcrumb-item active" aria-current="page"> <?php _e('正文', 'Charlotte'); ?></li>
                                </ol>
                            </div>
                        <?php } ?>
                        <div class="header">
                            ...
                            <div class="meta">
                                ...
                                <?php if (Charlotte_option('g_post_views', true)) { ?>
                                    <span><?php echo get_post_views();
                                            _e('点热度', 'Charlotte'); ?></span>
                                <?php }
                                if (Charlotte_option('g_post_loves', true)) { ?>
                                    <span><?php if (get_post_meta($post->ID, 'love', true)) {
                                                echo get_post_meta($post->ID, 'love', true);
                                            } else {
                                                echo '0';
                                            }
                                            _e('人点赞', 'Charlotte'); ?></span>
                                <?php }
                                if (Charlotte_option('g_post_comments', true)) { ?>
                                    <span><?php comments_number('0', '1', '%');
                                            _e('条评论', 'Charlotte'); ?></span>
                                <?php } ?>
                                <?php if (current_user_can('edit_posts')) {
                                    echo '<span>';
                                    edit_post_link(__('编辑文章', 'Charlotte'));
                                    echo '</span>';
                                }; ?>
                            </div>
                        </div>
                        <div class="content" id="lightgallery">
                            <?php
                            if (!empty(Charlotte_option('single_ad_top_group'))) {
                                foreach (Charlotte_option('single_ad_top_group') as $group_item) {
                            ...
                            if (!empty(Charlotte_option('single_ad_bottom_group'))) {
                                foreach (Charlotte_option('single_ad_bottom_group') as $group_item) {
                                    if ($group_item['ad_switcher']) {
                                        echo '<div style="margin-bottom:5px"><a href="' . $group_item['ad_url'] . '" target="_blank" rel="noreferrer"><img src="' . $group_item['ad_img'] . '"></a></div>';
                                    }
                                }
                        ...
                        <?php if (Charlotte_option('g_cc_fieldset')['g_cc_switch'] ?? false) {
                            $cc_array = array(
                            ...
                            printf(__('本作品采用 %s 进行许可', 'Charlotte'), $cc_array[Charlotte_option('g_cc_fieldset')['g_cc']]);
                            echo '</span></div>';
                        } ?>
                        <div class="footer clearfix">
                            <div class="tags float-left">
                                <span><?php _e('标签：', 'Charlotte'); ?></span>
                                <?php if (get_the_tags()) {
                                    the_tags('', ' ', '');
                                } else {
                                    echo '<a>' . __('暂无', 'Charlotte') . '</a>';
                                } ?>
                            </div>
                            <div class="tool float-right d-none d-lg-block">
                                <span><?php _e('最后更新：', 'Charlotte'); ?><?php the_modified_date(); ?></span>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <nav class="navigation post-navigation clearfix" role="navigation">
                    ...
                    if (!empty($prev_post)) {
                        echo '<div class="nav-previous clearfix"><a title="' . $prev_post->post_title . '" href="' . get_permalink($prev_post->ID) . '">' . __('< 上一篇', 'Charlotte') . '</a></div>';
                    }
                    $next_post = get_next_post(TRUE);
                    if (!empty($next_post)) {
                        echo '<div class="nav-next"><a title="' . $next_post->post_title . '" href="' . get_permalink($next_post->ID) . '">' . __('下一篇 >', 'Charlotte') . '</a></div>';
                    } ?>
                </nav>
                <?php comments_template(); ?>
            </div>
            <?php if (Charlotte_option('g_article_widgets', 'two_side') == 'two_side') { ?>
